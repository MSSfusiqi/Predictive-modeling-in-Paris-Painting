...
# Introduction: Summary of problem and objectives
...
## EDA of numeric, binary, and categorical variables

Since we do not initially know what predictors are important, we did some exploratory data analysis (EDA). For convenience, we divide the data frame  into numeric, binary, and categorical. First we looked at scatter plots of the mutual correlation between numeric variables and found the variables that are most correlated with ‘logprice’, what we want to predict. In this plot we did see any obvious linear relationship between predictors `logprice`. (Codes of the remaining EDA are in RMD)

```{r EDA for numeric, echo=FALSE}
...
We then plot the scatter plot matrix and boxplot for binary variables. From the boxplot, we find that some variables (`relig`,`arch`,`othartist`, `mytho`) are not significant in predicting `logprice`. We show the one of the visualization as follows (Codes of the remaining EDA are in RMD):


```{r EDA for binary,echo=FALSE}
...
pairs(as.matrix(train_binary[,c(1, 1:10)]))
#pairs(as.matrix(train_binary[,c(1, 11:15)]))
#pairs(as.matrix(train_binary[,c(1, 16:20)]))#pairs(as.matrix(train_binary[,c(1, 21:23)]))

```

We also plot the boxplot, and did not find any obvious variable which is not significant in predicting `logprice`. 

```{r EDA for categorical, echo=FALSE}
...
```{r additional eda ,include=FALSE,echo=FALSE}
for(i in 2:13){
...
We select ten variables using corrleation analysis as follows:
...
## AIC and BIC Stepwise Model Selection
